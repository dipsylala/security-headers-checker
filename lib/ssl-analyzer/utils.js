...
function logCertificateDebugInfo(cert, hostname, options = {}) {
    ...
    logger.info(`SSL Certificate Debug Information for ${hostname}:`);

    if (!cert) {
        logger.info('  No certificate available');
        return;
    }

    logger.info(`  Subject: ${cert.subject ? formatX509Name(cert.subject) : 'Unknown'}`);
    logger.info(`  Issuer: ${cert.issuer ? formatX509Name(cert.issuer) : 'Unknown'}`);
    logger.info(`  Valid From: ${cert.valid_from || 'Unknown'}`);
    logger.info(`  Valid To: ${cert.valid_to || 'Unknown'}`);
    logger.info(`  Serial Number: ${cert.serialNumber || 'Unknown'}`);
    logger.info(`  Fingerprint: ${cert.fingerprint || 'Unknown'}`);
    logger.info(`  Key Length: ${cert.bits || 'Unknown'} bits`);
    logger.info(`  Signature Algorithm: ${cert.sigalg || 'Unknown'}`);

    if (cert.subjectaltname) {
        logger.info(`  Subject Alternative Names: ${cert.subjectaltname}`);
    }

    if (cert.ext_key_usage) {
        logger.info(`  Extended Key Usage: ${cert.ext_key_usage.join(', ')}`);
    }

    if (cert.issuerCertificate && cert.issuerCertificate !== cert) {
        logger.info(`  Issuer Certificate Subject: ${formatX509Name(cert.issuerCertificate.subject)}`);
    }
}
