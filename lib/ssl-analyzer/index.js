 ...
 */
async function performSSLAnalysis(url) {
    ...
    try {
        ...
        // Optionally perform SSLyze analysis
        ...
        if (sslyzeAvailable.available) {
            ...
            const sslyzeResult = await sslyzeIntegration.runSSLyzeScan(hostname, port, { timeout: 90000 });
    ...
    if (results.basic) {
        ...
        if (results.basic.validTo) {
            ...
            tests.push({
                ...
                recommendation: daysUntilExpiry <= 30 ? 'Certificate expires soon. Plan for renewal.' : null
            });
        }
