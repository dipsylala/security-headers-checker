...
async function analyzeSSLCertificateDetailed(hostname, port = 443, options = {}) {
    console.log(`[SSL-ANALYZER] analyzeSSLCertificateDetailed called for ${hostname}:${port}`);
    console.log(`[SSL-ANALYZER] Received options:`, JSON.stringify(options, null, 2));
    console.log(`[SSL-ANALYZER] Fast mode check: options.fast = ${options.fast}, type = ${typeof options.fast}`);

    try {
        // Skip SSLyze if fast mode is enabled
        if (options.fast) {
            console.log(`[SSL-ANALYZER] ✅ Fast mode enabled - skipping SSLyze analysis for ${hostname}:${port}`);
        ...
        console.log(`[SSL-ANALYZER] ❌ Fast mode NOT enabled - running full SSLyze analysis for ${hostname}:${port}`);
        ...
        if (sslyzeAvailable.available) {
            console.log(`Comprehensive SSL analysis using SSLyze for ${hostname}:${port}`);
            ...
            if (sslyzeResult.success) {
                ...
                console.log(`SSLyze scan failed for ${hostname}:${port}: ${sslyzeResult.error}`);
            }
        }

        // Fallback to certificate-only analysis
        console.log(`Certificate-only SSL analysis for ${hostname}:${port} (SSLyze not available)`);
