...
function calculateSSLGrade(cert, protocol, authorized, signatureAlgorithm = null, authorizationError = null) {
    ...
    // Score TLS protocol version (0-30 points)
...
/**
 * Calculate comprehensive SSL score with detailed breakdown
 * @param {Object} sslAnalysis - Complete SSL analysis object
 * @returns {Object} Detailed scoring breakdown
 */
function calculateComprehensiveSSLScore(sslAnalysis) {
    const scores = {
        certificate: 0,
        protocol: 0,
        keyExchange: 0,
    ...
    if (sslAnalysis.valid) {
        ...
        // Bonus points for certificate features
        if (sslAnalysis.keyLength >= 2048) scores.certificate += 5;
        if (sslAnalysis.signatureAlgorithm && sslAnalysis.signatureAlgorithm.includes('SHA256')) scores.certificate += 5;
    }
    
    // Protocol score (30 points max)
    scores.protocol = scoreProtocol(sslAnalysis.protocol, [], []);
    
    // Key exchange score (15 points max) - simplified
    scores.keyExchange = 10; // Default moderate score
    
    // Cipher score (15 points max) - simplified  
    scores.cipher = 10; // Default moderate score
    
    scores.total = scores.certificate + scores.protocol + scores.keyExchange + scores.cipher;
    scores.grade = determineGrade(scores.total);
...
module.exports = {
    ...
    description: 'SSL/TLS certificate grading, scoring, and security assessment algorithms'
};
