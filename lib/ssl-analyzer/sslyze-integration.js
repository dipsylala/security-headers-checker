...
async function checkSSLyzeAvailability() {
    return new Promise((resolve) => {
        exec('sslyze --help', { timeout: 10000 }, (error, stdout, stderr) => {
            if (error) {
                resolve({
                    available: false,
                    error: error.message,
...
async function runSSLyzeScan(hostname, port = 443, options = {}) {
    ...
    return new Promise((resolve) => {
        const timeoutMs = options.timeout || 60000; // Increase timeout to 60 seconds
...
/**
 * Generate SSLyze scan report summary
 * @param {Object} sslyzeResults - SSLyze results
 * @returns {Object} Scan summary
 */
function generateSSLyzeSummary(sslyzeResults) {
    const summary = {
    ...
    try {
        // Check connectivity
        if (sslyzeResults.connectivityResult) {
            summary.serverReachable = true;
            summary.scanSuccessful = true;
        }
        
        // Convert to tests and analyze
        const tests = convertSSLyzeToTests(sslyzeResults);
        
        // Check certificate
        if (tests.certificateInfo && tests.certificateInfo.passed) {
            summary.certificateValid = true;
        }
        
        // Collect supported protocols
        ['ssl2Support', 'ssl3Support', 'tls1_0Support', 'tls1_1Support', 'tls1_2Support', 'tls1_3Support'].forEach(protocol => {
            if (tests[protocol] && tests[protocol].supported) {
                summary.protocolsSupported.push(protocol.replace('Support', ''));
            }
        });
        
        // Check vulnerabilities
        if (tests.heartbleed && tests.heartbleed.vulnerable) {
            summary.vulnerabilitiesFound.push('Heartbleed');
            summary.recommendedActions.push('Update OpenSSL to fix Heartbleed vulnerability');
        }
        
        if (tests.robot && tests.robot.vulnerable) {
            summary.vulnerabilitiesFound.push('ROBOT');
            summary.recommendedActions.push('Update TLS implementation to fix ROBOT vulnerability');
        }
        
        if (tests.ccsInjection && tests.ccsInjection.vulnerable) {
            summary.vulnerabilitiesFound.push('CCS Injection');
            summary.recommendedActions.push('Update OpenSSL to fix CCS Injection vulnerability');
        }
        
        // Protocol recommendations
        if (summary.protocolsSupported.includes('ssl2Support') || summary.protocolsSupported.includes('ssl3Support')) {
            summary.recommendedActions.push('Disable SSL 2.0 and SSL 3.0 protocols');
        }
        
        if (summary.protocolsSupported.includes('tls1_0Support') || summary.protocolsSupported.includes('tls1_1Support')) {
            summary.recommendedActions.push('Consider disabling TLS 1.0 and TLS 1.1');
        }
        
    } catch (error) {
        summary.error = `Summary generation failed: ${error.message}`;
    }
