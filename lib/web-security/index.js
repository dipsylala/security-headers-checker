...
async function performWebSecurityChecks(url) {
    ...
    logger.info(`Running ${enabledChecks.length} web security checks...`);

    try {
        // Run all checks in parallel for better performance
        const checkPromises = enabledChecks.map(async (checkConfig) => {
            try {
                logger.info(`Running ${checkConfig.module.name}...`);
                ...
                logger.info(`${checkConfig.module.name} completed in ${duration}ms`);
                return result;
            } catch (error) {
                logger.error(`Error in ${checkConfig.id} check: ${error.message}`);
        ...
        logger.error(`Additional checks error: ${error.message}`);
    }
