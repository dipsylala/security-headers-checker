...
function analyzeSSLyzeVulnerabilities(scanResult) {
    ...
    vulnChecks.forEach(vuln => {
        ...
        if (vuln.key === 'heartbleed') {
            ...
            const vulnerable = vulnResult.robot_result_enum && vulnResult.robot_result_enum !== 'NOT_VULNERABLE_NO_ORACLE';
            if (vulnerable) {
                status = 'fail';
                score = 0;
                recommendation = 'Update TLS implementation to fix ROBOT attack vulnerability';
                details = `ROBOT vulnerability: ${vulnResult.robot_result_enum || 'Unknown status'}`;
            } else {
                details = vulnResult.robot_result_enum ? 
                    'Server is not vulnerable to ROBOT attack' :
                    'ROBOT vulnerability status could not be determined';
            }
        }
...
function analyzeSSLyzeTLSFeatures(scanResult) {
    ...
    // TLS compression
    const compressionResult = scanResult.compression;
    if (compressionResult && compressionResult.status === 'COMPLETED' && compressionResult.result) {
        ...
        let compressionStatus = compression.compression_name !== 'DISABLED' ? 'warning' : 'pass';
        let compressionScore = compression.compression_name !== 'DISABLED' ? 2 : 5;
        let compressionRecommendation = compression.compression_name !== 'DISABLED' ? 
        ...
        tests.push({
            ...
            details: compression.compression_name !== 'DISABLED' ? 
...
async function performDetailedCertificateAnalysis(cert, protocol, authorized, signatureAlgorithm, authorizationError, hostname, port = 443) {
    ...
    const maxScoreWithSSLyze = 175 + 50; // Base + SSLyze potential scores
