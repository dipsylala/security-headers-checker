...
/**
 * Get all header scoring configuration
 * @returns {Object} Complete scoring configuration
 */
function getAllScoring() {
    const allScoring = {};
    HEADER_MODULES.forEach(module => {
        allScoring[module.category] = module.scoring;
    });
    return allScoring;
}
...
function getHeadersByCategory(category) {
    const module = HEADER_MODULES.find(m => m.category === category);
    return module ? module.headers : [];
}
...
function getHeaderCategories() {
    return HEADER_MODULES.map(module => module.category);
}
...
function getModuleInfo() {
    return HEADER_MODULES.map(module => ({
        category: module.category,
        name: module.name,
        description: module.description,
        headerCount: module.headers.length,
...
function calculateHeaderScore(header, value) {
    const allScoring = getAllScoring();
    const config = allScoring[header.category];
    if (!config) { return 0; }
...
/**
 * Get scoring summary for all categories
 * @returns {Object} Scoring configuration summary
 */
function getScoringSummary() {
    const allScoring = getAllScoring();
    const summary = {};
    
    Object.keys(allScoring).forEach(category => {
        const config = allScoring[category];
        const module = HEADER_MODULES.find(m => m.category === category);
        summary[category] = {
            ...config,
            actualHeaders: module ? module.headers.length : 0,
            maxPossibleScore: config.maxHeaders * config.points * config.weight
        };
    });
